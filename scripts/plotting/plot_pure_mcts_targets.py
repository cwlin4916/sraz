"""Figure for note 02 section 2: pure MCTS on lin_D and quad_B, R^2 vs budget.

Named for the targets it plots rather than for the sweep, because
`plot_pure_mcts_sweep.py` is a *different* experiment on the additive/sine
problems and the two must not share a path.
...
what separates "search found the answer" from "the tie-break did" -- and on
quad_B at 5 simulations it was the tie-break.
...
of length zero is invisible. A dot sitting on the zero line is not. Its rows
are in fixed production order (j = 0..6) rather than sorted by Q, so B1 and B2
share a y-axis and can be read against each other.
...
def draw_root(ax, data, target):
    # Fixed production order (j = 0..6, the grammar table of note 01 section 2),
    # not sorted by Q: identical y-axes are what make B1 and B2 comparable at a
    # glance. Rows with no dot are edges the search never visited -- that
    # absence is itself the finding on lin_D, so the row stays.
    rows = sorted(cell_for(data, target, ROOT_BUDGET)["root"],
                  key=lambda r: r["action"])
    rows = rows[::-1]                       # j = 0 at the top
    ypos = np.arange(len(rows))
    visited = [r for r in rows if r["Q"] is not None]
    win_row = max(visited, key=lambda r: r["N"])   # visit-count argmax selects
    win = rows.index(win_row)
    ...
    for y, r in zip(ypos, rows):
        q = r["Q"]
        if q is None:
            ax.text(0.06, y, "not visited", color=MUTED, fontsize=7.2,
        ...
        ax.plot([q], [y], "o", ms=9, color=colour, zorder=4,
                markeredgecolor="white", markeredgewidth=1.2)
        # Label on the far side of the dot from zero, so it never sits on a mark.
        ax.text(q + (0.09 if q >= 0 else -0.09), y, f"Q={q:+.2f}  N={r['N']}",
                color=INK if y == win else MUTED, fontsize=7.4, va="center",
    ...
    ax.set_title(f"{'B1' if target == 'lin_D' else 'B2'}   root edges after "
                 f"{ROOT_BUDGET} simulations   (shaded = selected)",
