...
def test_reuse_reinjects_fresh_noise():
    ...
    def expected_fresh_mix(rng_state):
        ...
        consumes exactly one ``np.random.dirichlet`` draw, so replaying from
        the pre-call state recovers exactly the injected noise."""
        np.random.set_state(rng_state)
        noise = np.random.dirichlet(
            [mcts.dirichlet_alpha] * len(original_before))
    ...
    rng_state_2 = np.random.get_state()
    mcts.perform_simulations_reuse(None, add_noise=True)
